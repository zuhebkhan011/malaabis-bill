...
const getReportsDashboard = async (req, res) => {
  try {
    ...
    const [todaySalesAgg, monthlySalesAgg, allBillsAgg, topProductsAgg, lowStockProducts, inventoryAgg, paymentAgg, chartAgg, salesBreakdownAgg] = await Promise.all([
      Bill.aggregate([
      ...
      Bill.aggregate([
        ...
        {
          $group: {
            _id: { $ifNull: ["$items.isManual", false] },
            revenue: { $sum: "$items.lineTotal" },
    ...
    // Sales Breakdown grouping parser
    const salesBreakdown = {
      inventory: { revenue: 0, quantity: 0 },
      manual: { revenue: 0, quantity: 0 }
    };
    salesBreakdownAgg.forEach((entry) => {
      if (entry._id === true) {
        salesBreakdown.manual = { revenue: entry.revenue || 0, quantity: entry.quantity || 0 };
      } else {
        salesBreakdown.inventory = { revenue: entry.revenue || 0, quantity: entry.quantity || 0 };
      }
    });
