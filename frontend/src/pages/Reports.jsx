        ...
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1.4fr_0.9fr] gap-6">
            ...
            <div className="space-y-6">
              ...
              {summary.salesBreakdown && (
                <SectionShell
                  title="Catalog vs Manual Sales Breakdown"
                  subtitle="Comparison of sales revenue from inventory products vs manually entered products."
                  action={<span className="material-symbols-outlined text-primary">pie_chart</span>}
                >
                  <div className="space-y-4">
                    {/* Catalog Products */}
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-on-surface font-medium uppercase tracking-wider text-xs">Catalog Products</span>
                        <span className="text-primary font-semibold">{formatCurrency(summary.salesBreakdown.inventory?.revenue || 0)} ({summary.salesBreakdown.inventory?.quantity || 0} units)</span>
                      </div>
                      <div className="h-3 rounded-full bg-white/5 overflow-hidden">
                        <div className="h-full rounded-full bg-primary transition-all duration-500" style={{
                          width: `${Math.round(((summary.salesBreakdown.inventory?.revenue || 0) / Math.max((summary.salesBreakdown.inventory?.revenue || 0) + (summary.salesBreakdown.manual?.revenue || 0), 1)) * 100)}%`
                        }} />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-on-surface font-medium uppercase tracking-wider text-xs">Manual Bill Items</span>
                        <span className="text-[#ffe88a] font-semibold">{formatCurrency(summary.salesBreakdown.manual?.revenue || 0)} ({summary.salesBreakdown.manual?.quantity || 0} units)</span>
                      </div>
                      <div className="h-3 rounded-full bg-white/5 overflow-hidden">
                        <div className="h-full rounded-full bg-[#ffe88a] transition-all duration-500" style={{
                          width: `${Math.round(((summary.salesBreakdown.manual?.revenue || 0) / Math.max((summary.salesBreakdown.inventory?.revenue || 0) + (summary.salesBreakdown.manual?.revenue || 0), 1)) * 100)}%`
                        }} />
                      </div>
                    </div>
                  </div>
